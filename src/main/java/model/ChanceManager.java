...
public class ChanceManager {
    private static Card ADVANCE_TO_RED3 = new MoveToPropertyCard(String.format("Advance to %1$s", SpaceManager.RED3_NAME), (PurchasableSpace) SpaceManager.RED3);
    private static Card ADVANCE_TO_NEAREST_UTILITY = new AdvanceToNearestCard("Advance token to nearest utility\nIf unowned, you may buy it from the bank\nIf owned, throw dice and pay owner a total 10 times the amount thrown", PurchasableType.UTILITY);
    private static Card ADVANCE_TO_NEAREST_STATION = new AdvanceToNearestCard("Advance token to nearest station and pay owner twice the rental to which he/she is otherwise entitled\nIf station is unowned, you may buy it from the bank", PurchasableType.STATION);
    private static Card ADVANCE_TO_NEAREST_STATION_SECOND = new AdvanceToNearestCard("Advance token to nearest station and pay owner twice the rental to which he/she is otherwise entitled\nIf station is unowned, you may buy it from the bank", PurchasableType.STATION);
    ...
    private static Card PAY_ALL_PLAYERS = new PayAllPlayersCard("You have been elected chairman of the board pay each player $500", 50);
    private static Card RIDE_TO_STATION1 = new MoveToPropertyCard(String.format("Take a ride to %1$s\nIf you pass GO collect $200", SpaceManager.STATION1_NAME), (PurchasableSpace) SpaceManager.STATION1);
    ...
    private static Card GENERAL_REPAIR = new PayPerHouseCard("Make general repairs on all your property\nFor each house pay $25\nfor each hotel $100", 25, 100);
    private static Card ADVANCE_TO_BLUE2 = new MoveToPropertyCard(String.format("Take a walk on %1$s\nAdvance token to %2$s", SpaceManager.GREEN3_NAME, SpaceManager.BLUE2_NAME), (PurchasableSpace) SpaceManager.BLUE2);
    private static Card ADVANCE_TO_PINK1 = new MoveToPropertyCard(String.format("Advance to %1$s\nIf you pass GO, collect $200", SpaceManager.PINK1_NAME), (PurchasableSpace) SpaceManager.PINK1);
    private static Card CASH_IN_INVESTMENTS = new ReceiveCard("Cash in on investments\nCollect $150", 150);
    private static Card GO_TO_JAIL = new GoToJailCard("Go directly to jail\nDo not pass go\nDo not collect $200");
